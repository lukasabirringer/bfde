...
// beachfelder Stylesheet build
gulp.task('pl-build:beachfelder-stylesheet', function () {
  return run('npm run build:all:dev').exec();
});

// beachfelder Stylesheets Copy
gulp.task('pl-copy:beachfelder-stylesheet', gulp.series('pl-build:beachfelder-stylesheet', function () {
  return gulp.src(resolvePath(paths().source.beachfelderStylesheet) + '/dist/**/*')
    .pipe(gulp.dest(resolvePath(paths().public.css)))
    .pipe(browserSync.stream());
}));
...
gulp.task('pl-assets', gulp.series(
  gulp.parallel(
    ...
    'pl-copy:beachfelder-stylesheet'
  ),
  function (done) {
    done();
  })
...
function watch() {
  ...
  gulp.watch(resolvePath(paths().source.beachfelderStylesheet) + '/src/**/*.scss', { awaitWriteFinish: true }).on('change', gulp.series('pl-copy:beachfelder-stylesheet', reloadCSS));
